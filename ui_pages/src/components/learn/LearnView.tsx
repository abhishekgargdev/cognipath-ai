...
export const LearnView: React.FC = () => {
  const { selectedTopicId, startDailyPractice } = useApp();
  const lesson = mockLessons[selectedTopicId] || mockLessons['js-closures'];
  ...
  return (
    <div id="learn-view" className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-200">
      {/* Lesson Header Banner (Monograph Frontispiece) */}
      <div className="p-6 sm:p-8 rounded-xs border border-[#DCD9D1] dark:border-[#2C2A26] bg-[#FFFFFF] dark:bg-[#181714] shadow-xs relative overflow-hidden border-t-3 border-t-[#8B2635] dark:border-t-[#E08A95]">
        <div className="flex flex-wrap items-center gap-2 mb-3 text-xs">
          ...
          <span className="text-[#9E9A91]">•</span>
          <span className="text-[#5C5852] dark:text-[#9E9A91] font-mono text-[11px]">Difficulty: {lesson.difficulty}</span>
          <span className="text-[#9E9A91]">•</span>
          <span className="text-[#121212] dark:text-[#F4F2EC] font-mono text-[11px] font-bold">{lesson.masteryLevel}% Mastery Index</span>
        </div>
        ...
        {/* Why you're learning this callout (Editorial Pull-Quote) */}
        ...
        <div className="mt-5 flex items-center justify-between pt-3 border-t border-[#DCD9D1] dark:border-[#2C2A26]">
          <div className="flex items-center gap-2 text-xs text-[#5C5852] dark:text-[#9E9A91] font-mono">
            ...
            {lesson.prerequisites.map(p => (
              <span key={p} className="px-2 py-0.5 rounded-xs bg-[#F4F1EA] dark:bg-[#201F1B] border border-[#DCD9D1] dark:border-[#2C2A26] text-[10px] font-medium text-[#121212] dark:text-[#F4F2EC]">
                {p}
              </span>
            ))}
          </div>
          ...
          <button
            onClick={() => startDailyPractice(lesson.topicId)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        {/* Left 3 Columns: Lesson Narrative, Visual Diagram, Code Snippets */}
        <div className="lg:col-span-3 space-y-10">
                ...
                <div className="rounded-xs border border-[#DCD9D1] dark:border-[#2C2A26] bg-[#121212] overflow-hidden shadow-xs">
                  {section.codeSnippet.caption && (
                    <div className="px-4 py-2 border-b border-[#2C2A26] bg-[#1A1916] text-xs text-[#A6A299] font-mono flex items-center justify-between">
                      ...
                      <span className="text-[10px] uppercase tracking-widest">{section.codeSnippet.language}</span>
                    </div>
                  )}
          ...
          {/* CONCEPT VISUALIZATION: Under the hood runtime diagram */}
          <div id="visualization" className="space-y-4 pt-4">
            <h2 className="text-2xl font-serif font-bold text-[#121212] dark:text-[#F4F2EC] border-b border-[#DCD9D1] dark:border-[#2C2A26] pb-2">
              Concept Architecture: The Scope Closure Chain
            </h2>
            <p className="text-xs font-serif italic text-[#5C5852] dark:text-[#A6A299] leading-relaxed">
              Optical breakdown of how the Call Stack delegates closed-over variables to the Heap:
            </p>
            ...
            <div className="p-5 rounded-xs border border-[#DCD9D1] dark:border-[#2C2A26] bg-[#FFFFFF] dark:bg-[#181714] font-mono text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-xs border border-[#DCD9D1] dark:border-[#2C2A26] bg-[#F4F1EA] dark:bg-[#201F1B]">
                  ...
                  <div className="space-y-2">
                    <div className="p-2.5 rounded-xs bg-[#FFFFFF] dark:bg-[#141412] border border-[#DCD9D1] dark:border-[#2C2A26] text-[#121212] dark:text-[#F4F2EC]">
                      [1] ExecutionContext: createCounter() <span className="text-[#8B2635] dark:text-[#E08A95] text-[10px] font-bold">(Exited & Popped!)</span>
                    </div>
                    <div className="p-2.5 rounded-xs bg-[#FFFFFF] dark:bg-[#141412] border border-[#DCD9D1] dark:border-[#2C2A26] text-[#121212] dark:text-[#F4F2EC]">
                      [2] Global Execution Context (Active)
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xs border border-[#8B2635]/40 dark:border-[#E08A95]/40 bg-[#8B2635]/5 dark:bg-[#8B2635]/10">
                  <div className="text-[10px] font-bold text-[#8B2635] dark:text-[#E08A95] uppercase tracking-[0.2em] mb-2">
                    Heap Memory (Persisted via Closure)
                  </div>
                  <div className="p-3.5 rounded-xs border border-[#8B2635]/20 bg-[#FFFFFF] dark:bg-[#141412]">
                    <div className="text-[#8B2635] dark:text-[#E08A95] font-bold text-[11px] mb-1">LexicalEnvironmentRecord</div>
                    <div className="text-[#121212] dark:text-[#F4F2EC] space-y-0.5 text-[11px]">
                      <div>let count: 2;</div>
                      <div className="text-[#5C5852] dark:text-[#9E9A91] text-[10px]">Reference count: 2 (increment, getValue)</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-[#DCD9D1] dark:border-[#2C2A26] text-center text-[#5C5852] dark:text-[#9E9A91] text-[11px] font-serif">
                Garbage collector check: Reachable from live function pointers? <strong className="text-[#8B2635] dark:text-[#E08A95]">YES</strong> → Retained in Heap.
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 font-mono text-xs">
                    ...
                    <div className="p-3.5 rounded-xs border border-[#8B2635]/30 bg-[#8B2635]/5 dark:bg-[#8B2635]/10">
                      <span className="text-[9px] uppercase font-bold tracking-widest text-[#8B2635] dark:text-[#E08A95] block mb-1">The Bug</span>
                    ...
                    <div className="p-3.5 rounded-xs border border-[#1F3A2B]/30 bg-[#1F3A2B]/5 dark:bg-[#1F3A2B]/10">
                      <span className="text-[9px] uppercase font-bold tracking-widest text-[#1F3A2B] dark:text-[#4E876A] block mb-1">The Fix</span>
                      <pre className="text-[#121212] dark:text-[#F4F2EC] whitespace-pre-wrap">
          ...
          <div id="knowledge-check" className="space-y-4 pt-4">
            <div className="flex items-center justify-between border-b border-[#DCD9D1] dark:border-[#2C2A26] pb-2">
              ...
              <span className="text-xs font-mono text-[#5C5852] dark:text-[#9E9A91] uppercase tracking-wider">2 Checkpoint Questions</span>
            </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      {kc.options.map((opt, oIdx) => {
                        ...
                        let optStyle = 'border-[#DCD9D1] dark:border-[#2C2A26] bg-[#FFFFFF] dark:bg-[#141412] hover:bg-[#F4F1EA] dark:hover:bg-[#201F1B] text-[#121212] dark:text-[#F4F2EC]';
                        if (isAnswered) {
                    ...
                    {isAnswered && (
                      <div className={`p-3 rounded-xs text-xs font-serif leading-relaxed ${
                        isCorrect 
        ...
        <div className="hidden lg:block">
          <div className="sticky top-24 p-4 rounded-xs border border-[#DCD9D1] dark:border-[#2C2A26] bg-[#FFFFFF] dark:bg-[#181714] shadow-xs space-y-3">
            ...
            <nav className="space-y-1.5 text-xs font-serif">
              <a href="#intro" className="block text-[#5C5852] dark:text-[#B5B1A7] hover:text-[#121212] dark:hover:text-[#F4F2EC] transition-colors">
                Introduction
              </a>
              <a href="#what-is-closure" className="block text-[#5C5852] dark:text-[#B5B1A7] hover:text-[#121212] dark:hover:text-[#F4F2EC] transition-colors">
                What is a Closure?
              </a>
              <a href="#how-it-works" className="block text-[#5C5852] dark:text-[#B5B1A7] hover:text-[#121212] dark:hover:text-[#F4F2EC] transition-colors">
                Under the Hood & Heap
              </a>
              <a href="#real-world" className="block text-[#5C5852] dark:text-[#B5B1A7] hover:text-[#121212] dark:hover:text-[#F4F2EC] transition-colors">
                Real-World Debounce
              </a>
